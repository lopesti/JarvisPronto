...
// Carregar bot config
const botConfig = JSON.parse(fs.readFileSync(path.join(__dirname, '../../bot_volumetrao.json'), 'utf8'));
...
async function generateResponse(userInput, history = []) {
    ...
    try {
        ...
        logger.warn('Groq falhou, usando Gemini:', error.message);
        try {
            ...
            return botConfig.fallbackMessage || 'Desculpe, estou com problemas técnicos. Tente novamente mais tarde.';
        }
    }
}
